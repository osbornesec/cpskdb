        ...
        if result.returncode != 0:
            ...
            self.assertTrue(
                any(
                    ...
                    for term in ["99999", "invalid", "range"]
                ),
...
"""
        ...
        self.compose_file = self.setup_compose_file(
            compose_content_network_conflict, self.temp_dir
        )
...
services:
  qdrant:
    ...
    volumes:
      - qdrant_data:/qdrant/storage:ro  # Read-only mount prevents Qdrant from writing
    command: ["./qdrant", "--config-path", "/nonexistent/config/missing.yaml"]  # Guaranteed to fail
...
"""
        ...
        self.compose_file = self.setup_compose_file(
            compose_content_startup_fail, self.temp_dir
        )
    ...
    def test_error_messages_include_relevant_context(self):
        ...
        if response.status_code >= 400:
            try:
                ...
                self.assertTrue(
                    len(error_text) > 0, "Error response should not be empty"
                )
...
"""
        ...
        self.compose_file = self.setup_compose_file(
            compose_content_warnings, self.temp_dir
        )
    ...
    def test_api_error_response_format_consistency(self):
        ...
        error_tests = [
            ...
            (
                ...
                {"vector": "invalid_vector", "limit": "not_a_number"},
            ),
        ...
        if error_formats:
            ...
            for error_format in error_formats:
                ...
                has_error_info = any(
                    key in error_format
                    for key in ["error", "message", "detail", "status", "text"]
                )
